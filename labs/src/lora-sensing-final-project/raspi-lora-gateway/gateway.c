...
// name of file where the lora packets are logged
// original
// const char *FileLoraMsg = "packets.txt";
const char *FileLoraMsg = "packets.csv";
//-------------------------------------------
...
// original log packets
void LogPacketOriginal( int Channel, int8_t SNR, int RSSI, double FreqError, int Bytes, unsigned char MessageType )
{
...
// 4: RSSI rx packet
// 5: FreqErr rx packet
// 6: n.Bytes of str message
// 7: string message
//
void LogPacket( int Channel, int8_t SNR, int RSSI, double FreqError, int Bytes, char *StrMessage )
{
    if ( Config.EnablePacketLogging )
    {
        FILE *fp;

        if ( ( fp = fopen( FileLoraMsg, "at" ) ) != NULL )
        {
            time_t now;
            struct tm *tm;
            ...
            fprintf(fp,
                    "%04d/%02d/%02d %02d:%02d:%02d,%d,%d,%d,%.1lf,%d,%s\n",
                    1900 + tm->tm_year, 
                    tm->tm_mon, 
                    tm->tm_mday, 
                    tm->tm_hour, 
...
receiveMessage( int Channel, char *message )
{
    ...
    else
    {
        ...
        // mr: mod 28/07
		// update csv with lora packets
		LogPacket(Channel, PacketSNR(Channel), PacketRSSI(Channel), FreqError, Bytes, message );

		// tuning lora frequency
